...
module Problems11To20 =
    ...
    *)
    let solve11() =
        let getSequenceByMatrix productMatrix (grid : int [,]) row col =
            Seq.map (fun (x, y) -> grid.[row + x, col + y]) productMatrix

        let mapMatrix size mapping =
            seq {
        ...
        let getGroupProducts productMatrix (grid : int [,]) =
            let getProduct i j = 
                getSequenceByMatrix productMatrix grid i j 
                |> Math.intProduct
            
            mapMatrix 20 getProduct
                        
        let getHorizontalProducts = getGroupProducts [(0,0);(0,1);(0,2);(0,3)]

        let getVerticalProducts = getGroupProducts [(0,0);(1,0);(2,0);(3,0)]

        let getDownDiagonalProducts = getGroupProducts [(0,0);(1,1);(2,2);(3,3)]

        let getUpDiagonalProducts = getGroupProducts [(3,0);(2,1);(1,2);(0,3)]
        ...
        [|
            ...
            getVerticalProducts
            getDownDiagonalProducts
            getUpDiagonalProducts
        |]
        |> Seq.collect (fun f -> f grid)
        |> Seq.max